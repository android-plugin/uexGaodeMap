...
public class GaodeMapMarkerMgr extends GaodeMapBaseMgr implements OnMarkerClickListener, OnInfoWindowClickListener, InfoWindowAdapter {
	...
	private HashMap<String, InfoWindowAdapter> mLayouts = new HashMap<String, InfoWindowAdapter>();
	private ImageLoaderManager manager;
	private List<LatLng> mOverlays;

	public GaodeMapMarkerMgr(Context cxt, AMap map, OnCallBackListener listener, List<LatLng> markers) {
		super(cxt, map, listener);
		manager = ImageLoaderManager.initImageLoaderManager(mContext);
		this.mOverlays = markers;
		new BaseHandler(Looper.getMainLooper());
	}
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (bean.getCustomBubble() != null) {
				...
				default:
					...
					CustomBubbleMarkerLayout layout = new CustomBubbleMarkerLayout(mContext, item);
	...
	private String getMarkerId(String markerID) {
		String id = null;
		Iterator<String> it = mMarkers.keySet().iterator();
		while (it.hasNext()) {
